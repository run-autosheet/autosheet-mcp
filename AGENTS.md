...
- **Root-package listing metadata is validated by OpenAI only when the package is uploaded as a ZIP** — `interface.displayName` and `interface.shortDescription` are capped at 30 characters, `brandColor` needs 2:1 contrast against white, and `logo`/`composerIcon` are required on that path. The build script enforces the text limits; the public directory listing (the **With MCP** submission) is a portal form that never reads the manifest. `docs/plugin.md` records the full set; check it before editing `extensions.com.openai.interface` in `plugin.json`.
