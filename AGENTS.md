...
- `mcp.json` at the root — `type: "streamable-http"`, the Agent Plugins 1.0 format, used by Codex and every other Agent Plugins client for the root package. The build script converts it to `type: "http"` in a generated `.mcp.json` that ships inside the OpenAI ZIP.
